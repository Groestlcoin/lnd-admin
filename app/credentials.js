...
if (process.env.LND_ADMIN_NODE_COUNT) {
	for (var i = 1; i <= parseInt(process.env.LND_ADMIN_NODE_COUNT); i++) {
		...
		var adminMacaroonFilepath = process.env["LND_ADMIN_MACAROON_FILEPATH_" + i] || path.join(os.homedir(), '.lnd-grs', 'admin.macaroon');
		var tlsCertFilepath = process.env["LND_ADMIN_TLS_CERT_FILEPATH_" + i] || path.join(os.homedir(), '.lnd-grs', 'tls.cert');
	...
	rpcConfigs.push({
		...
		adminMacaroonFilepath:process.env.LND_ADMIN_ADMIN_MACAROON_FILEPATH || path.join(os.homedir(), '.lnd-grs', 'admin.macaroon'),
		tlsCertFilepath:process.env.LND_ADMIN_TLS_CERT_FILEPATH || path.join(os.homedir(), '.lnd-grs', 'tls.cert'),
	});
}
